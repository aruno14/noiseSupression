...
class MySequence(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        while current_size < batch_size:
            ...
            for k in range(0, min(len(spectNoisy), len(spectClear))-voice_max_length-1):
                batch_x_train[current_size] = spectNoisy[k:k+voice_max_length]
                batch_y_train[current_size] = spectClear[k+voice_max_length+1]
                current_size+=1
...
plt.legend(['mse'])
plt.savefig("learning-lstm_simple.png")
plt.show()
plt.close()
...
for i, path_clear in enumerate(clear_files):
    ...
    for i in range(0, len(spectNoisy)-voice_max_length):
        input[i] = spectNoisy[i:i+voice_max_length]
...
for test_path in [('data/noisy/book_00000_chp_0009_reader_06709_0_---1_cCGK4M.wav'), ('data/noisy/book_00000_chp_0009_reader_06709_1_---1_cCGK4M.wav')]:
    ...
    for i in range(0, len(spect_real)-voice_max_length):
        input[i] = spect_real[i:i+voice_max_length]
    print("input.shape:", input.shape)
    result_gain = model.predict(input)
    print("result_gain.shape:", result_gain.shape)
    result_gain = tf.concat([tf.zeros((voice_max_length, int(frame_length/2+1))), result_gain], axis=0)
    print("result_gain.shape:", result_gain.shape)
    oscillo = spectToOscillo(spect_real=result_gain, spect_sign=spect_sign, spect_image=spect_image, audioSR=16000)
